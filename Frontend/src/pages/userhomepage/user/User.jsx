  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'profile':
        return <Profile />;
      case 'settings':
        return <p className="text-lg">Settings: Customize your settings here.</p>;
      case 'viewSchedule':
        return <UserSchedule />;
      case 'membership':
        return <Membership />;
  ...
  return (
    <div className="flex flex-col md:flex-row h-full min-h-screen bg-gray-100 p-4">
      <div className="w-full md:w-1/4 lg:w-1/5 p-4 bg-white shadow-lg rounded-lg mb-4 md:mb-0">
        ...
        <button
          className={`w-full py-2 mb-2 rounded-lg ${activeTab === 'settings' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'} transition-all`}
