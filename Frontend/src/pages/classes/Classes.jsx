...
const Classes = () => {
  ...
  useEffect(() => {
    const fetchClassesAndPT = async () => {
      try {
        ...
        // Fetch classes với phân trang
        const responseClasses = await fetch(
          `http://localhost:3000/classes/paginated?page=${currentPage}&limit=${limit}`
        );
        const data = await responseClasses.json();
        
        // Lọc các lớp có status_id === 2
        const filteredClasses = data.data.filter((classItem) => classItem.status_id === 2);
  
        // Fetch user count cho mỗi lớp
        const classesWithCountsPromises = filteredClasses.map(async (classItem) => {
    ...
    fetchClassesAndPT();
  }, [currentPage, limit, searchParams]); // Thêm searchParams vào dependencies
  ...
  const searchClasses = async (searchTerm) => {
    if (!searchTerm.trim()) {
      ...
      return data.data.filter((classItem) => classItem.status_id === 2);
    }
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/classes/search?${params}`);
