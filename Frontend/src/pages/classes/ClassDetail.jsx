...
const ClassDetail = () => {
  ...
  const handleJoinClass = async () => {
    ...
    try {
      ...
      // Kiểm tra điều kiện trước khi cho join
      const canJoin = await checkClassConditions();
      if (!canJoin) {
        console.log('Cannot join class due to conditions not met');
        return;
      }

      // Nếu có phí, chuyển hướng đến thanh toán
      if (fee > 0) {
        handlePayment();
        return;
      }
      
      // Nếu là lớp miễn phí, tạo payment với amount = 0
      const paymentData = {
      ...
      if (response?.payment?.order_url) {
        ...
        throw new Error('Không thể tạo yêu cầu tham gia lớp học');
      }
  ...
  const handlePayment = async () => {
    try {
        ...
        setJoinStatus(prev => ({
            ...prev,
            isLoading: false,
            error: error.message || 'Không thể xử lý thanh toán'
        }));
    } finally {
        setJoinStatus(prev => ({ ...prev, isLoading: false }));
    }
        ...
        content = (
          <>
            <p className="text-yellow-700">
              Membership của bạn sẽ hết hạn vào {new Date(notification.details.membershipEnd).toLocaleDateString('vi-VN')},
              trong khi lớp học kết thúc vào {new Date(notification.details.classEnd).toLocaleDateString('vi-VN')}
            </p>
            <p className="mt-1 text-yellow-700">
              Bn cần thêm {notification.details.monthsNeeded} tháng membership để hoàn thành lớp học này
            </p>
            <button
              onClick={() => navigate('/membership')}
              className="mt-3 inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-yellow-700 bg-yellow-100 hover:bg-yellow-200"
    ...
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
        <div className="flex items-start">
          <svg className="w-5 h-5 text-yellow-400 mt-1 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    try {
      ...
      if (!membershipData.isActive) {
        setNotification({
          show: true,
          type: 'membership',
          message: 'Bạn cần đăng ký Membership để tham gia lớp học',
          details: null
        });
        setHasMembership(false);
        return false;
      ...
      if (membershipData.membershipDetails) {
        ...
        if (membershipEnd < classEnd) {
          ...
          setNotification({
            ...
            message: 'Membership của bạn sẽ hết hạn trước khi lớp học kết thúc',
            details: {
              membershipEnd,
              classEnd,
              monthsNeeded
      ...
      if (!classData.schedule) {
        console.log('Lớp học không có lịch');
        setNotification({
          show: false,
          type: '',
          message: '',
      ...
      const scheduleData = {
        ...
        start_hour: classData.schedule.start_hour,
        end_hour: classData.schedule.end_hour,
        class_id: parseInt(classId)
      };

      // Kiểm tra tính hợp lệ của dữ liệu lịch
      if (!scheduleData.days || !scheduleData.start_hour || !scheduleData.end_hour) {
        console.log('Lớp học không có đủ thông tin lịch');
        setNotification({
          show: false,
          type: '',
          message: '',
      ...
      if (scheduleResult.conflictingClasses && scheduleResult.conflictingClasses.length > 0) {
          setNotification({
            show: true,
            type: 'schedule',
            message: 'Ca học này đã có lịch học trùng với lịch học của bạn',
          details: scheduleResult.conflictingClasses
          });
          return false;
      }
      ...
      setNotification({
        show: true,
        type: 'error',
        message: 'Có lỗi xảy ra khi kiểm tra điều kiện',
        details: null
      });
      return false;
    }
  ...
  const formattedFee = fee ? `$${parseFloat(fee).toFixed(2)}` : 'No fee specified';
                      ...
                      {joinStatus.isLoading ? (
                        ...
                        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                          <div className="flex items-center">
                            ...
                            <p className="text-green-700 font-medium">Bạn đã đăng ký khóa học này!</p>
                          </div>
                        </div>
                      ) : isMax ? (
                        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                          <div className="flex items-center">
                            ...
                            <p className="text-red-700 font-medium">
                              Lớp học đã đầy ({userCount}/{classData.maxAttender} chỗ)
                            </p>
                          </div>
                        </div>
                      ) : (
